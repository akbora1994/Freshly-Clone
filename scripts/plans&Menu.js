...
function displayMeals(data) {
  ...
  data.map(function (data) {
    ...
    box.setAttribute("class", "meals-div");
    var card = document.createElement("div");
    card.setAttribute("class", "meals-card");
    ...
    meal.textContent = data.meal;
    meal.setAttribute("class", "mealbox");

    card.append(img, name, meal);
    box.append(card);
...
planCards.map(item => {
  ...
  saving.setAttribute("class", "savingbox");
...
function filtercategory() {
  var filter = document.querySelector("#meal-filter").value;

  var filteredList = data.filter(function (data) {
    console.log("filter", data.category, filter)
    return data.category === filter;
  });
  displayMeals(filteredList);
}
